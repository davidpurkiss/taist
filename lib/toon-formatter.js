...
export class ToonFormatter {
  ...
  /**
   * Group traces by correlationId (preferred) or traceId (fallback).
   *
   * Correlation IDs persist across async boundaries (e.g., Apollo Server),
   * allowing traces from different async contexts to be grouped together
   * if they belong to the same HTTP request.
   *
   * @param {Array} traces - Array of trace objects
   * @returns {Map<string, Array>} - Map of correlationId/traceId to traces
   */
  groupTracesByRequest(traces) {
    const groups = new Map();
    for (const trace of traces) {
      // Prefer correlationId for grouping, fall back to traceId
      const id = trace.correlationId || trace.traceId || 'unknown';
