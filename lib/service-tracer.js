...
export class ServiceTracer extends EventEmitter {
  ...
  wrapMethod(method, fullName, methodName) {
    ...
    return async function(...args) {
      ...
      const traceId = tracer.enter(fullName, args);
      stats.totalCalls++;
      ...
      try {
        ...
        if (options.captureSlowOps && duration > options.slowOpThreshold) {
          ...
          tracer.event('slow_operation', {
            method: fullName,
            duration,
            args: args.slice(0, 2) // Limited args for safety
          });
          emit('slow-operation', { method: fullName, duration, args });
        }

        // Detect bugs based on patterns
        if (options.detectBugs) {
          checkForBugs(fullName, methodName, args, result, options, tracer, stats);
        }
        ...
        if (options.captureErrors) {
          emit('error', { method: fullName, error, args });
        }
    ...
    function checkForBugs(fullName, methodName, args, result, options, tracer, stats) {
      // Check custom bug patterns
      for (const [bugName, checker] of Object.entries(options.bugPatterns)) {
        if (typeof checker === 'function') {
          try {
            if (checker(methodName, args, result)) {
              stats.bugsDetected++;
              tracer.event(`bug:${bugName}`, {
                method: fullName,
                args: args.slice(0, 2),
      ...
      if (methodName === 'validateEmail' && args[0]?.includes('+')) {
        if (result === true || !result) {
          stats.bugsDetected++;
          tracer.event('bug:email_plus_sign', {
            email: args[0],
            message: 'Email with + sign passed validation'
          });
        }
      }

      if (methodName === 'getStats' && result?.cacheRatio !== undefined) {
